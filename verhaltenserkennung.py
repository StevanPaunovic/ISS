...
def get_predicted_label(result):
    """
    Extracts the predicted label from the inference result.

    :param result: Result from `inference_recognizer`
    :return: Predicted label ID as an integer
    """
    if hasattr(result, "pred_label"):
        return int(result.pred_label.item())
    else:
        raise ValueError("Das Ergebnis enthält kein gültiges `pred_label` oder ist leer.")
...
def erkennung(file: str):
    config_path = 'C:/Users/steva/Desktop/mmaction2/configs/recognition/tsn/tsn_imagenet-pretrained-r50_8xb32-1x1x8-100e_kinetics400-rgb.py'
    ...
    pred_scores = result.pred_score.cpu().numpy()  # Convert tensor to numpy array
    sorted_indices = pred_scores.argsort()[::-1]  # Indices of scores sorted in descending order
    
    log_content = f"Analyse für Datei: {file}\n"
    log_content += "Top 5 vorhergesagte Aktivitäten:\n"
    
    for i in range(5):
        class_id = sorted_indices[i]
        class_name = activity_dict.get(class_id, f"Unbekannte Klasse ({class_id})")
        probability = pred_scores[class_id]
        log_content += f"{class_name}: {probability:.4f}\n"
        print(f"{class_name}: {probability:.4f}")
    
    log_content += "\n"
    save_results_to_file(log_file, log_content)
